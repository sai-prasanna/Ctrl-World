...
| `scripts/eval_video_metrics.py` | Rolls out a batch of clips and computes the metrics. |
...
the model does. The evaluation therefore scores two frozen-frame baselines against the
same targets:

- `psnr_static_round` repeats the frame each round was conditioned on. It answers whether
  the model beat freezing the image for that second.
- `psnr_static_first` repeats the clip's initial observation for the whole rollout. It
  shows how long the model stays better than doing nothing.

Both anchor on a real frame, so the baseline does not degrade as the model drifts. The
comparison is reported as `_gain_db`, the difference in decibels. PSNR is already

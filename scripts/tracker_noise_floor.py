...
def static_mask_from_variance(frames, percentile, margin=8):
    """Boolean (H, W) marking the quietest pixels, away from the frame border."""
...
def main():
    ...
    for clip in clips:
        ...
        for view in args.views:
            ...
            queries = seeding.grid_queries(h, w, grid=args.grid)
            err, tracks, vis = cycle_error(tracker, clip_frames, queries)
            displacement = np.linalg.norm(tracks - tracks[0][None], axis=-1).max(axis=0)

            mask, var = static_mask_from_variance(clip_frames, args.static_percentile)
            static_q = seeding.mask_queries(mask, n_points=64, seed=0)
            ...
            record['views'][view] = {
                ...
                'static_pixel_variance_cutoff': float(
                    np.percentile(var, args.static_percentile)),
            }

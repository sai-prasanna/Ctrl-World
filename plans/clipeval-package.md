...
Keep the package inside this repo for now, under a top-level `clipeval/` directory, and make it
pip-installable later if a second project needs it. Publishing early costs a release process
...
- Which environment to install CoTracker3 into. `torch` is not importable in the base shell, so
  the repo uses a conda environment or a virtualenv that this plan does not yet name.
